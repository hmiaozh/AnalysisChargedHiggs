...
const int bkgtype = 3;
const int proalltype = 15;
const int prosepatype = 14;    //6--discr;13--sepa type
const int colorbkg[bkgtype] = {kBlue-10,kGreen-10,kOrange-9};
TString bkgname[bkgtype] = {"TT_TuneCUETP8M2T4_13TeV-powheg-pythia8","Other","QCD_HT"};
TString bkgshort[bkgtype] = {"ttbar","other","QCD"};
TString proname[proalltype] = {"HT","NBjets","Njets","NFatjets","NSubJetB","NSubJetBLoose","Met","SDMass","top_pt","W_pt","fat_pt","tau32","tau21","ele_pt","muon_pt"};
TString prosepaname[prosepatype] = {"HT","invariantExt","NBjets","Njets","NFatjets","fat_pt","W_pt","TAU32","TAU21","PtTopH","PtB1","softdropM","nbsubjet","nbsubjetloose"};
...
void bujiu(){
	...
	TFile* f1 = TFile::Open("/afs/cern.ch/work/h/hum/public/CMSSW_8_0_26_patch1/src/ChargedHiggs/ntufile/startoverQCD.root");
...
//		hRatio->GetYaxis()->SetNdivisions(502);
		...
		c1->SaveAs(Form("/afs/cern.ch/work/h/hum/public/CMSSW_8_0_26_patch1/src/ChargedHiggs/Analysis/Plots/2018Mar/mar07_QCD_0b1W_%s.png",proname[i].Data()));
	}
*/
      for(int k=0; k<4; k++){
	for(int i=5; i<prosepatype-8; i++){
...
//		if( (k==2 || k==3) && (i==6 || i ==8)) continue;  
		...
		else if(i>=5 && i<11) ht_data->Rebin(5);  
//		ht_data->Rebin(5);
                ht_data->SetMarkerColor(1);
                ht_data->SetLineColor(1);
                ht_data->SetMarkerSize(1);
...
//              hRatio->GetYaxis()->SetNdivisions(502);
		hRatio->Draw("P E X0 SAME");
                ...
                c1->SaveAs(Form("/afs/cern.ch/work/h/hum/public/CMSSW_8_0_26_patch1/src/ChargedHiggs/Analysis/Plots/2018Mar/mar07_QCD_mirror_%s_cate%i.png",prosepaname[i].Data(),k));
        }
     }
