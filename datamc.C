...
const int prosepatype = 16;    //6--discr;13--sepa type
...
TString proname[proalltype] = {"ST","HT","NBjets","Njets","NFatjets","NSubJetB","NSubJetBLoose","Met","SDMass","top_pt","W_pt","fat_pt","tau32","tau21","ele_pt","muon_pt"};
TString prosepaname[prosepatype] = {"ST","HT","invariantExt","NBjets","Njets","NFatjets","fat_pt","top_pt","W_pt","TAU32","TAU21","PtTopH","PtB1","softdropM","nbsubjet","nbsubjetloose"};
...
//TString prosepaname[prosepatype] = {"ST","HT","invariantExt","NBjets","Njets","NSubJet","subCSV1","subCSV2","subCSVmax","DRtb","PtTopH","PtB1","softdropM","nbsubjet","nbsubjetloose"};
...
void datamc(){
	...
	TFile* f1 = TFile::Open("/afs/cern.ch/work/h/hum/public/CMSSW_8_0_26_patch1/src/ChargedHiggs/ntufile/zerobfinal.root");

	TDirectoryFile* all = (TDirectoryFile*)f1->Get("ChargedHiggsTopBottom/NoB");
	TDirectoryFile* allbug = (TDirectoryFile*)f1->Get("ChargedHiggsTopBottom/NoB");
	TDirectoryFile* tb[4];
	TDirectoryFile* tbbug[4];
/*
	tb[0] = (TDirectoryFile*)f1->Get("ChargedHiggsTopBottom/OneBOneFat1l_t0b");
	tb[1] = (TDirectoryFile*)f1->Get("ChargedHiggsTopBottom/OneBOneFat1l_t1b");
	tb[2] = (TDirectoryFile*)f1->Get("ChargedHiggsTopBottom/OneBOneFat1l_wbb");
	tb[3] = (TDirectoryFile*)f1->Get("ChargedHiggsTopBottom/OneBOneFat1l_wbj");

	tbbug[0] = (TDirectoryFile*)f1->Get("ChargedHiggsTopBottom/OneBOneFat1l_t0b");
        tbbug[1] = (TDirectoryFile*)f1->Get("ChargedHiggsTopBottom/OneBOneFat1l_t1b");
        tbbug[2] = (TDirectoryFile*)f1->Get("ChargedHiggsTopBottom/OneBOneFat1l_wbb");
        tbbug[3] = (TDirectoryFile*)f1->Get("ChargedHiggsTopBottom/OneBOneFat1l_wbj");
*/
...
//		pad_up->SetTopMargin(0.05);
		...
		hRatio->Divide(hbkg);
	//	for(int q = 0; q < 60; q++)
	//		cout << "QCDHT_Wtype_Reweighting spline " << (q+1)*80. << " " << hRatio->GetBinContent(q+1) << " " << 0 << endl;
...
//		hRatio->GetYaxis()->SetNdivisions(502);
		...
		c1->SaveAs(Form("/afs/cern.ch/work/h/hum/public/CMSSW_8_0_26_patch1/src/ChargedHiggs/Analysis/Plots/2018Mar/mar26_QCD_0b_%s.png",proname[i].Data()));
	}
/*
      for(int k=0; k<4; k++){
...
//		if( (k==2 || k==3) && (i==6 || i ==8)) continue;  
		...
		else if(i>=6 && i<13) ht_data->Rebin(5);  
//		ht_data->Rebin(5);
                ht_data->SetMarkerColor(1);
                ht_data->SetLineColor(1);
                ht_data->SetMarkerSize(1);
...
//              hRatio->GetYaxis()->SetNdivisions(502);
		hRatio->Draw("P E X0 SAME");
                ...
                c1->SaveAs(Form("/afs/cern.ch/work/h/hum/public/CMSSW_8_0_26_patch1/src/ChargedHiggs/Analysis/Plots/2018Mar/mar14_tt1l_%s_cate%i.png",prosepaname[i].Data(),k));
        }
     }
